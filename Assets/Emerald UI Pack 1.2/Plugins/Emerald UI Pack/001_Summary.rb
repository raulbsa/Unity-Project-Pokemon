...
class PokemonSummary_Scene
  ...
  def pbStartScene(party, partyindex, inbattle = false)
    ...
    @sprites["shinybg"].setBitmap("Graphics/UI/Summary/overlay_shiny")
  ...
  def drawPage(page)
    ...
    # Show shiny icon
    if @pokemon.shiny?
      imagepos.push([sprintf("Graphics/UI/shiny"), 44, 346])
    end
    ...
    if status >= 0
      imagepos.push(["Graphics/UI/statuses", 94, 348, 0, 16 * status, 44, 16])
    end
    # Show Pokérus cured icon
    if @pokemon.pokerusStage == 2
      imagepos.push([sprintf("Graphics/UI/Summary/icon_pokerus"), 52, 348])
    end
    # Draw all images
  ...
  def drawPageOne
    ...
    if @pokemon.shadowPokemon?
      ...
      imagepos = [
        ["Graphics/UI/Summary/overlay_shadow", 212, 260],
        ["Graphics/UI/Summary/overlay_shadowbar", 236, 286, 0, 0, (shadowfract * 248).floor, -1]
      ]
      pbDrawImagePositions(overlay, imagepos)
    end
    # Write various bits of text
    ...
    if @pokemon.shadowPokemon?
      ...
      if @pokemon.level < GameData::GrowthRate.max_level
        ...
        pbDrawImagePositions(overlay,
                            [["Graphics/UI/Summary/overlay_exp", 252, 364, 0, 0, w, 6]])
      end
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
  ...
  def drawPageThree
    ...
    Pokemon::MAX_MOVES.times do |i|
      ...
      if move
        ...
        imagepos.push(["Graphics/UI/types", 222, yPos - 4, 0, type_number * 28, 64, 28])
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    imagepos = [["Graphics/UI/category", 138, 348, 0, selected_move.display_category(@pokemon) * 22, 66, 22]]
    pbDrawImagePositions(overlay, imagepos)
